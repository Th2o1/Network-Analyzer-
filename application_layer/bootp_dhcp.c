...
// Function to print the flags in the IP header
// Function to print BOOTP flags
void print_bootp_flags(uint16_t flags) {
    printf("Flags:");
    if(verbosity == HIGH) printf("0x%02x ", ntohs(flags));

    // Check individual flags
    if (ntohs(flags) & 0x8000) {
        printf("(Broadcast) ");
    } else {
        printf("(Unicast) ");
    }
...
void parse_bootp(const u_char *packet, size_t offset) {
    ...
    printf("BOOTP/DHCP: ");
    printf("Operation: %s, ", bootp_header->bp_op == BOOTREQUEST ? "REQUEST" : "REPLY");
    printf("length: %zu, ", packet_size - offset);
    if(verbosity == LOW){return;} // we stop here for low verbosity
    printf("ID: 0x%08x, ", ntohl(bootp_header->bp_xid));
    const u_char *packet_bootp = (u_char *)bootp_header;
    uint16_t flags = (packet_bootp[10] << 8) | packet_bootp[11];
    print_bootp_flags(flags);
    ...
    if(verbosity == HIGH){
        printf("\n");
        printf("Hardware Type: %s, ", bootp_header->bp_htype == 1 ? "Ethernet" : "Unknown");
        ...
        if (bootp_header->bp_file[0] != '\0') printf("Server Host Name: %.64s, ", bootp_header->bp_sname);
    
        if (bootp_header->bp_file[0] != '\0') printf("Boot File Name: %.128s, ", bootp_header->bp_file);
    }
